...
function ff(a: number, b: number, c: number, d: number, x: number, s: number, ac: number): number {
	let value = addUnsigned(a, addUnsigned(addUnsigned(f(b, c, d), x), ac))
	value = rotateLeft(value, s)
	return addUnsigned(value, b)
}

function gg(a: number, b: number, c: number, d: number, x: number, s: number, ac: number): number {
	let value = addUnsigned(a, addUnsigned(addUnsigned(g(b, c, d), x), ac))
	value = rotateLeft(value, s)
	return addUnsigned(value, b)
}

function hh(a: number, b: number, c: number, d: number, x: number, s: number, ac: number): number {
	let value = addUnsigned(a, addUnsigned(addUnsigned(h(b, c, d), x), ac))
	value = rotateLeft(value, s)
	return addUnsigned(value, b)
}

function ii(a: number, b: number, c: number, d: number, x: number, s: number, ac: number): number {
	let value = addUnsigned(a, addUnsigned(addUnsigned(i(b, c, d), x), ac))
	value = rotateLeft(value, s)
	return addUnsigned(value, b)
}
...
function convertToWordArray(source: string): number[] {
	const words: number[] = []
	...
	while (byteIndex < messageLength) {
		...
		const current = wordIndex < words.length ? words[wordIndex] : 0
		const code = source.charCodeAt(byteIndex)
		words[wordIndex] = current | (code << bytePosition)
		byteIndex += 1
	}
	wordIndex = byteIndex >> 2
	const bytePosition = (byteIndex % 4) * 8
	const current = wordIndex < words.length ? words[wordIndex] : 0
	words[wordIndex] = current | (0x80 << bytePosition)
	words[(((byteIndex + 8) >> 6) + 1) * 16 - 2] = messageLength << 3
	words[(((byteIndex + 8) >> 6) + 1) * 16 - 1] = messageLength >>> 29
	return words
}
...
function wordToHex(value: number): string {
	...
	let index = 0
	while (index <= 3) {
		const part = (value >>> (index * 8)) & 255
		const hex = '0' + part.toString(16)
